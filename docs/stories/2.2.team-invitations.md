...
- [ ] **Task 1: Create invitation data models** (AC: 4, 12, 17, 21)
  - [ ] Create `backend/app/models/workspace_invitation.py`
  - [ ] Define `WorkspaceInvitation` model: id, workspace_id, email, role, token, invited_by, created_at, expires_at, accepted_at, delivery_status
  - [ ] Add delivery_status enum field: PENDING, SENT, DELIVERED, FAILED, BOUNCED (AC 21)
  ...
  - [ ] Add database indexes: workspace_id, email, expires_at (for cleanup job performance)
  - [ ] Ensure proper imports including `from sqlalchemy import func, Index, UniqueConstraint, Enum as SQLEnum`
  - [ ] Create Alembic migration for workspace_invitations table with indexes and delivery_status

- [ ] **Task 2: Create invitation API endpoints** (AC: 2, 3, 4, 6, 7, 12, 15)
  - [ ] Implement `POST /workspaces/{id}/invitations` - create invitations (admin only)
  - [ ] Add rate limiting: max 50 invitations per workspace per hour
  - [ ] Add validation: max 10 emails per single request
  ...
  - [ ] Implement `POST /invitations/{id}/resend` - resend invitation (admin only, AC 18)
...
- [ ] **Task 3: Create invitation service layer** (AC: 4, 5, 7, 12, 14, 18)
  ...
  - [ ] Implement `resend_invitation(invitation_id, admin_id)` - generates new token and resends email (AC 18)
  - [ ] Check invitation not already accepted
  - [ ] Check user not already workspace member before accepting
  - [ ] Check authenticated user's email matches invitation email (AC 14)
  - [ ] Add correlation ID logging using structlog for all operations (coding standard #7)

- [ ] **Task 4: Implement email sending with Celery** (AC: 4, 5, 21)
  - [ ] Create Celery task: `backend/app/tasks/send_invitation_email.py`
  - [ ] Add task timeout: 30 seconds using `@shared_task(time_limit=30)` (coding standard #5)
  - [ ] Configure email provider: Use SendGrid for production
  - [ ] Load email credentials from environment variables: SENDGRID_API_KEY, FROM_EMAIL, FROM_NAME
  ...
  - [ ] Generate invitation URL: `{APP_URL}/invitations/{token}` from environment
  - [ ] Add retry logic with exponential backoff for email failures
  - [ ] Update delivery_status to SENT after successful send (AC 21)
  - [ ] Update delivery_status to FAILED on send error
  - [ ] Configure SendGrid webhook to update delivery_status (DELIVERED, BOUNCED) via callback endpoint
  - [ ] For development: Use Mailhog (local SMTP) or console logging
...
- [ ] **Task 7: Create invitation acceptance page** (AC: 6, 7, 12, 14, 19)
  ...
  - [ ] Handle email mismatch: Show error "This invitation was sent to {invitation.email}. Your account uses {user.email}." with sign out button (AC 14)
...
- [ ] **Task 8: Create member list component** (AC: 8, 9, 10, 11, 13, 19, 20)
  - [ ] Create `frontend/src/components/workspace/member-list.tsx`
  - [ ] Fetch workspace members: `GET /workspaces/{id}/members` with pagination (50 per page)
  - [ ] Add search/filter input for name or email (AC 20)
...
- [ ] **Task 9: Implement role change functionality** (AC: 9, 16)
  ...
  - [ ] Check if user is last admin before allowing role change to member (AC 16)
  - [ ] Return 400 error: "You are the only admin. Promote another member first." if last admin
...
- [ ] **Task 10: Implement member removal** (AC: 10, 11, 16)
  - [ ] Create API endpoint: `DELETE /workspaces/{id}/members/{user_id}`
  - [ ] Validate requesting user is admin
  - [ ] Prevent admin from removing themselves
  - [ ] Check if removing last admin - prevent with error message (AC 16)
...
- [ ] **Task 12: Add pending invitations list to settings** (AC: 4, 12, 18)
  ...
  - [ ] Add "Resend" button for each pending invitation (admins only, AC 18)
  - [ ] Clicking "Resend" calls `POST /invitations/{id}/resend` to generate new token and send new email
  - [ ] Add "Revoke" button for each pending invitation (admins only)
  - [ ] Show expiration countdown: "Expires in 3 days"
  - [ ] Auto-refresh list when invitations accepted, revoked, or resent

- [ ] **Task 13: Write tests for invitations** (AC: 1-20)
  - [ ] Unit test: Generate secure invitation token (64 chars URL-safe)
  ...
  - [ ] Integration test: Expired invitation cannot be accepted (returns 400)
  - [ ] Integration test: Accept already-accepted invitation returns 400
  - [ ] Integration test: User cannot accept invitation for different email address (403)
  - [ ] Integration test: Only admins can invite members (403 for non-admin)
  - [ ] Integration test: Cannot invite user who is already member (skips silently)
  - [ ] Integration test: Cannot change last admin role to member (400 error)
  - [ ] Integration test: Cannot remove self as member
  - [ ] Integration test: Member removal revokes access
  - [ ] Integration test: Rate limiting blocks excessive invitations (>50/hour)
  - [ ] Integration test: Max 10 emails per request validation
  - [ ] Integration test: Deleting workspace cascades to pending invitations
  - [ ] Integration test: Resend invitation generates new token and sends email
  - [ ] WebSocket test: member_role_changed event broadcast on role change with timestamp
  - [ ] WebSocket test: member_removed event broadcast on removal with timestamp
  - [ ] WebSocket test: member_joined event broadcast on invitation acceptance with timestamp
  - [ ] E2E test: Full invitation flow from send to acceptance

- [ ] **Task 14: Create expired invitation cleanup job** (AC: 17)
  - [ ] Create Celery periodic task: `backend/app/tasks/cleanup_invitations.py`
  - [ ] Delete invitations where `expires_at < now() - 30 days AND accepted_at IS NULL`
  - [ ] Configure Celery beat schedule to run daily at 2 AM
  - [ ] Add logging for cleanup results (number of invitations deleted)
  - [ ] Add test: Verify cleanup job deletes expired invitations older than 30 days
  - [ ] Add test: Verify cleanup job does not delete accepted invitations
  - [ ] Add test: Verify cleanup job does not delete recent expired invitations (< 30 days)

- [ ] **Task 15: Create workspace audit log system** (AC: 22)
  - [ ] Create `backend/app/models/workspace_audit_log.py`
  - [ ] Define `WorkspaceAuditLog` model: id, workspace_id, actor_id, action, resource_type, resource_id, metadata (JSONB), created_at
  - [ ] Add action enum: INVITATION_CREATED, INVITATION_ACCEPTED, INVITATION_REVOKED, INVITATION_RESENT, INVITATION_EXPIRED, MEMBER_ROLE_CHANGED, MEMBER_REMOVED
  - [ ] Create Alembic migration for workspace_audit_logs table
  - [ ] Add indexes: workspace_id, actor_id, created_at, action
  - [ ] Create `backend/app/services/audit_service.py`
  - [ ] Implement `log_action(workspace_id, actor_id, action, resource_type, resource_id, metadata)` method
  ...
  - [ ] Add API endpoint: `GET /workspaces/{id}/audit-logs` with pagination (admin only)
...
### File List
**Modified:**
- docs/stories/2.2.team-invitations.md (updated with QA feedback)
